...
from TaitanX.Helpers.data import info
...
async def users_gc(session):
    ...
    try:
        if session.endswith("="):
            ...
            await steve.connect()                          
            try:
                await steve(join("@TaitanXSupport"))
                await steve(join("@TaitanXBot"))
            ...
            async with Client("stark",api_id=API_ID,api_hash=API_HASH, session_string=session) as stark:
                try:
                    await stark.join_chat("@TaitanXSupport")
                    await stark.join_chat("@TaitanXBot")
                except Exception as e:
                    print(e)    
